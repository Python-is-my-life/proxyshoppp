...
async def proxy_main(message):
	...
	await bot.send_photo(chat_id=message.from_user.id, photo=PROXY_IMAGE, caption='<b>💬 Выберите действие:</b>',reply_markup = await proxy_main_keyboard())
...
async def send_balance(message, state):
	try:
		if message.text.isdigit():
			...
			cursor = conn.cursor()
			cursor.execute('UPDATE users SET balance = balance + (?) WHERE user_id = (?)', (int(message.text), int(user_id)))
			conn.commit()
			conn.close()
			await bot.send_message(chat_id = user_id, text=f'Ваш баланс пополнен на {message.text}₽', reply_markup = await menu_keyboard(int(user_id)))
...
async def get_proxy_type(call):
	#await bot.delete_message(chat_id= call.from_user.id, message_id= call.message.message_id)
	await bot.edit_message_caption(chat_id=call.from_user.id, message_id= call.message.message_id, caption='<b>🔍 Выберите тип прокси:</b>', reply_markup= await get_proxy_type_keyboard())
...
async def set_proxy_type(call, state=SET_PROXY_PARAMS):
	...
	await bot.edit_message_caption(chat_id=call.from_user.id, message_id= call.message.message_id, caption='<b>🌎 Выберите страну:</b>', reply_markup = await get_country_keyboard(0))

@dp.callback_query_handler(text_contains = 'next_country')
@dp.throttled(anti_flood,rate=0)
async def next_country(call):
	i = call.data[13:]
	await bot.edit_message_caption(chat_id=call.from_user.id, message_id=call.message.message_id, caption='<b>🌎 Выберите страну:</b>', reply_markup= await get_country_keyboard(i))
...
async def set_proxy_country(call, state=SET_PROXY_PARAMS):
	...
	#await bot.delete_message(chat_id = call.from_user.id, message_id= call.message.message_id)
	await bot.edit_message_caption(chat_id=call.from_user.id, message_id= call.message.message_id, caption='<b>🏙 Выберите город:</b>', reply_markup = await get_city_keyboard(country_code))
...
async def set_proxy_city(call, state=SET_PROXY_PARAMS):
	...
	#await bot.delete_message(chat_id = call.from_user.id, message_id= call.message.message_id)
	await bot.edit_message_caption(chat_id=call.from_user.id, message_id= call.message.message_id, caption='<b>🕔 Выберите период (ДНИ)</b>',reply_markup = await get_period_keyboard())
...
async def set_proxy_period(call, state=SET_PROXY_PARAMS):
	...
	all_count = requests.get(f'https://panel.proxyline.net/api/ips-count/?api_key={PROXY_KEY}&ip_version={ipv}&type={types}&country={country}').json()
	print(all_count)
	...
	if all_count !=0:
		...
		await bot.send_photo(chat_id=call.from_user.id, photo=PROXY_IMAGE, caption=f'<b>✏️ Введите количество прокси, которое вы хотите приобрести:</b>\n<i>🔎 Доступно: {all_count} шт</i>')
	...
	try:
		if message.text.isdigit():
			...
			if int(message.text) <= int(count_proxy):
				...
				info_ip = requests.get(f'https://panel.proxyline.net/api/ips/?api_key={PROXY_KEY}',params=params).json()
				print(info_ip)
				ip_list = []
				i = 0 
				for ip in info_ip:
				...
				info = requests.post(f'https://panel.proxyline.net/api/new-order-amount/?api_key={PROXY_KEY}', data=params).json()
				#print(info)
				if 'non_field_errors' not in info:
					keyboard = types.InlineKeyboardMarkup(row_width=2)
					price = await getRates(info.get('amount')) + await DB_GET_NACENKA()
					keyboard.add(types.InlineKeyboardButton(text=f'Оплатить {price} ₽', callback_data=f'buy_proxy|{price}'))
...
🕔 <b>Период:</b> <i>{5} дней</i>
						...
						'''.format(infolist['type'], infolist['ipv'], infolist['country_name'], infolist['city_name'], len(ip_list), infolist['period']), reply_markup=keyboard)
					await state.reset_state(with_data=False)
				else:
					await menu(message.from_user.id)
					await bot.send_message(chat_id=message.from_user.id, text= '<b><i>😥 В данный момент нет свободный серверов в этом городе\n⏳ Попробуйте позже</i></b>')
					await state.finish()
			else:
				await bot.send_message(chat_id=message.from_user.id, text= '<b>😥 Вы превысили кол-во доступных прокси\n✏️ Введите количество еще раз или нажмите "Отмена" </b>')
		...
		if float(price) <= await DB_GET_BALANCE(call.from_user.id):
			...
			if 'non_field_errors' in info:
				...
				if 'Not enough money on balance' in e_text:
					await bot.send_message(ADMIN, text='⚠️ <b>Не хватает баланса на сайте для покупки прокси\n\n⚠️Пополните баланс</b>')
				...
				for i in info:
					...
					cursor.execute('INSERT INTO proxy_list (user_id, ids, ip, http_port, socks5_port, username, password, date_end) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', (call.from_user.id, ids, ip, http_port, socks5_port, username, password, date_end[:10]))
					conn.commit()
					proxy_info = f'''
...
async def get_my_proxy(call):
	conn = sqlite3.connect('db.db', check_same_thread=False)
	cursor = conn.cursor()
	row = cursor.execute('SELECT ids, ip, date_end FROM proxy_list WHERE user_id = (?)', (call.from_user.id,)).fetchall()
	btn = []
	if row != []:
		keyboard = types.InlineKeyboardMarkup(row_width=2)
		for i in row:
	...
	if btn != []:
		...
		await call.message.answer('<b>📁 Список активных прокси:</b>\n<i>🕔 По истечению срока они удалятся из списка</i>', reply_markup=keyboard)
	else:
